...
export const getbydateTransaksi = async (request: Request, response: Response) => {
    try {
        const { startDate, endDate } = request.query;

        // Fungsi parsing tanggal dengan opsi akhir hari
        const parseDate = (dateString: string, isEndOfDay = false) => {
        ...
        const start = startDate ? parseDate(startDate.toString()) : null;
        const end = endDate ? parseDate(endDate.toString(), true) : new Date();

        // Siapkan objek filter tanggal hanya jika ada nilai valid
        const dateFilter: Record<string, any> = {};
        if (start) dateFilter['gte'] = start;
        if (end) dateFilter['lte'] = end;

        const whereClause = Object.keys(dateFilter).length > 0 
            ? { tgl_transaksi: dateFilter } 
            : {};

        // Query database dengan filter yang telah disiapkan
        const allTransaksi = await prisma.transaksi.findMany({
            where: whereClause,
            include: {
        ...
        const transaksiWithNota = allTransaksi.map((transaksi) => {
            ...
            return {
                id_transaksi: transaksi.id_transaksi,
                tgl_transaksi: transaksi.tgl_transaksi,
                nama_user: transaksi.user.nama_user,
